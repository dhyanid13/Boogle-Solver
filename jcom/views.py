...
import requests
def button(request):
    return render(request,'./jcom/home.html')

def output(request):
    return render(request,'./jcom/select.html')
...
def output1(request):
    ...
    a=(arr(letters))
#    print(a)
    with open('C:/Users/Lenovo/DATASET/board.txt', 'w') as f:
        for item in a:
            f.write('{}\n'.format(" ".join(item)))
    f.close()
    data=a
    return render(request,'./jcom/4x4.html',{'data':a})
...
#    print(a)
    ...
    f.close()
    data=a
    return render(request,'./jcom/5x5.html',{'data':a})
    
def addword(request):
    word=request.POST.get('word')
    #RENDERING OUTPUT
    return render(request,'./jcom/4x4.html',{'e':word})
...
def output3(request):
    ...
    def print_res():
        ...
        print(scoreans);
    res=print_res()
    return render(request,'5x5.html',{'res':res})
